                    ...
                    if 'NGSTAR_profiles.tab' in p:
                        if st != 'ST':
                            ...
                            if 'NG-STAR_CC' not in linesplit:
                                print(
                                    '## NG-STAR CCs not in the database. Update the database with -u including the CCs file with -cc. Exiting...')
                                sys.exit()
...
def assign_sts_only(filename, outfilename, schemes, profilesDB, MLSTorder, NGSTARorder, NGMASTorder, ngstarccs,
                    ngstarCCsdic, mosaic_pena, penAmosaicsdic):
    resultsvec = []
    with open(filename, 'r') as tab:
        header = next(tab)
        if '>' in header:
            print(
                '## Input is a fasta file, a table with typing profiles is required to assign sequence types. Exiting...')
            sys.exit()
        sep = '\t' if '\t' in header else ','
...
def blast_newalleles(query, subject, path):
    ...
    # run blastn
    blast_cmd = ['blastn', '-out', 'tmp/' + subject_name + '.blastn', '-outfmt', '6', '-query', path + '/' + query,
                 '-db', 'tmp/' + subject_name, '-evalue', '0.001', '-max_target_seqs', '1']
...
def ac_fast(fname, seq, order, allelesDB, allelesAC, genogroups, out_path, PORout_results, TBPBout_results):
    results = {}
    subset_dict = defaultdict(lambda: {})
    for i in order:
        results[i] = ['-']
    for end_index, (insert_order, original_value) in allelesAC.iter(seq):
        ...
        if gene in order:
            subset_dict[gene][allele] = original_value
            if results[gene] == ['-']:
                results[gene] = [allele]
            else:
                if gene in subset_dict and results[gene][0] in subset_dict[gene]:
                    old_seq = subset_dict[gene][results[gene][0]]
                if original_value in old_seq:
                    pass
                elif old_seq in original_value:
                    results[gene] = [allele]
                else:
                    results[gene].append(allele)
            # save POR and TBPB sequences if genogroups are requested
            if genogroups:
                save_ngmast_genes(fname, original_value, gene, allele, direction, out_path, PORout_results,
                                  TBPBout_results)
    return results
...
def process_files(args):
    ...
    if not fpath.endswith('.fasta') or fpath.endswith('.fas') or fpath.endswith('.fa') or fpath.endswith('.fna'):
        print(
            '## Input file is not recognised as a fasta file. Please, include fasta files ending in .fasta, .fas, .fa or .fna')
        sys.exit()
    with open(fpath, 'r') as fasta:
        ...
        for record in SeqIO.parse(fasta, 'fasta'):
            concat += record.seq
    # AC algorithm:
    resultsDB = ac_fast(fname, str(concat), order, allelesDB, allelesAC, genogroups, out_path, PORout_results,
                        TBPBout_results)
